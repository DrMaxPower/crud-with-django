...
then worked on in that order. This strategy limits the work on:
- **What is my vision**   
to 
- **What should be done**  
and  
- **What developers think is realistic**  
to  
- **What customers, stakeholders and project leaders think are good**
...
- create the ability to send reservations to the Restaurant via the website
...
and the documentation is not done yet. The documentation should be done first even though I 
...
The information can have cross information with other tables, and how this relationship is set up in the Model. Django set up SQL code for the ability to talk to the databases automatically.
...
class="s12 m6" meaning in small devices using full width where medium size or bigger use half the width.
...
### Index
The index page contains information, extra links, and parallax images. A big part of Materialize mentality is to present a 3D illusion on a 2D plane and parallax helps to create the depth.
...
### Account 
Login, logout, and signup are generated from Django-allauth and designed with Materialize.
The decorator @login_required restricts a hacker to access /restaurant by adding it to the URL.
...
- add confirmation that the users booking has been done successfully. Preferably by sending an email or SMS confirmation.
...
In Django/settings the static files were redirected to Cloudinary and sweden-restaurant.herokuapp.com is an ALOWED_HOST. DEBUG = False will reject Django from putting out a small Bible of text every time something is wrong. This limit pirates from accessing sensitive content easily.  
